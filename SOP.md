...
| Kimi | 图表 HTML 生成 + 评审（备） | 按 Brief 生成可视化；也可做交叉评审给出第二意见 | 写文字内容、定调架构 |
| MiniMax M2.7 | 图表质量评审（主）+ 评审（多角色） | 25 分制图表打分、指出缺陷、提修改建议；也可做内容评审 | 生成图表 HTML |
...
**沉淀规则**：每次出现 `_LSOpenURLsWithCompletionHandler` / `secret scanner` / `404 on prod` / 跨子 Agent 冲突这几类问题，必须更新本表，不允许只在某次对话里讲完就忘。
