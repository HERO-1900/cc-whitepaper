...
| 模型 | 角色 | 典型任务 | 绝对不做 |
|------|------|---------|---------|
| Opus | 调度 + 顶级评审 | 写 Brief、综合多份报告、定夺架构决策 | 生成图表 HTML、单章写作 |
| Sonnet | 内容执行 | 写章节草稿、跑测试用户评审、做代码修复 | 做全局战略判断 |
| Kimi | 图表 HTML 生成 | 按 Brief 生成可视化 | 做评审、写文字 |
| MiniMax M2.7 | 图表质量评审 | 25 分制打分、指出缺陷、提修改建议 | 生成图表 |

**核心原则**：Opus 只做"只有 Opus 才能做"的事。Brief 写清楚之后，Opus 就退出任务链，由下游模型接手。
