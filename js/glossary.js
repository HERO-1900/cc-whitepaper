...
(function () {
  ...
  function renderDifficultyBadge(container, filePath) {
    ...
    badge.innerHTML = `<span class="difficulty-stars" title="阅读难度 ${stars}/5${note ? ' — ' + escapeHTML(note) : ''}">${starStr}${emptyStr}</span><span class="difficulty-label">${escapeHTML(friendly)}友好度</span>`;
  ...
  function renderQAPanel(container, filePath) {
    ...
    const levelLabels = { concept: '基础理解', extend: '延伸思考', critical: '批判反思' };
    ...
    let html = `
      <div class="qa-panel">
        <div class="qa-panel-header" role="button" tabindex="0">
          ...
          <span class="qa-panel-title">思考题 · ${qa.qa.length} 道</span>
    ...
    qa.qa.forEach((item, i) => {
      ...
      const diff = item.difficulty ? `<span class="qa-difficulty">难度 ${'●'.repeat(item.difficulty)}${'○'.repeat(5 - item.difficulty)}</span>` : '';
