...
**结论**：5+4 层的设计中，大约 2-3 层覆盖了 90% 的使用场景。剩下的层级是为企业市场的"长尾"需求准备的——它们的存在不会增加普通用户的复杂性（对外完全隐藏），但为 Anthropic 进入企业市场提供了必要的基础设施。这更像是一项"战略投资"而非"过度设计"，前提是这些额外的层级不会引入维护负担和 bug 风险——后文第 6 节分析的 first-source-wins 对象级粒度限制和缓存失效策略的简陋表明这个前提并非完全成立。
...
`SettingsSchema`（`types.ts:255+`）是一个庞大的 Zod schema，定义了约 **78 个顶级设置字段**（其中约 13 个受 feature flag 门控，如 `voiceEnabled`、`assistant`、`autoMode` 等，只在对应功能启用时出现；其余约 65 个为所有用户可见）。关键字段包括：
...
### 4.1 四个 `allowManaged*Only` 开关

| 开关 | 效果 |
|------|------|
| `allowManagedHooksOnly` | 只运行企业策略中的 Hooks，忽略用户/项目/本地 Hooks |
| `allowManagedPermissionRulesOnly` | 只尊重企业策略中的权限规则（allow/deny/ask） |
| `allowManagedMcpServersOnly` | MCP 白名单只读企业策略（但用户仍可 deny） |
| `strictPluginOnlyCustomization` | 指定的表面（skills/agents/hooks/mcp）只能通过插件定制 |
