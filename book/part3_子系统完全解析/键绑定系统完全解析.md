...
- **CodeX（OpenAI）**：底层已用 Rust 重写（95.6%），键绑定为硬编码的固定集合，无自定义能力。**OpenCode** 作为 Go+Zig 实现的 TUI 工具，通过 Tab 键切换 Build/Plan 模式，交互设计更接近传统终端应用。
...
支持丰富的修饰键别名：`ctrl`/`control`、`alt`/`opt`/`option`/`meta`、`cmd`/`command`/`super`/`win`。这确保了跨平台和不同用户习惯的兼容性。
...
和弦绑定（如 `ctrl+k ctrl+s`）需要跨按键的状态追踪。`resolveKeyWithChordState` 实现了一个三阶段决策流程：
...
用户按下 ctrl+k
     │
     ▼
Ink 框架产生 { input: 'k', key: { ctrl: true, ... } }
     │
     ▼
getKeyName() → 'k'
matchesKeystroke() → 检查修饰键匹配
     │
     ▼
resolveKeyWithChordState(input, key, ['Chat', 'Global'], bindings, null)
     │
     ├─ 找到更长和弦前缀 (ctrl+k ctrl+s 存在)
...
resolveKeyWithChordState(input, key, ['Chat', 'Global'], bindings, [ctrl+k])
     │
     ├─ 精确匹配 ctrl+k ctrl+s → 'chat:killAgents'
...
3. **同步加载 + 异步热重载**：初始加载是同步的（`readFileSync`），因为它在 React 的 `useState` 初始化器中调用。后续的热重载是异步的。两条路径的代码几乎完全重复（`loadKeybindings` 和 `loadKeybindingsSyncWithWarnings`），维护成本高。
