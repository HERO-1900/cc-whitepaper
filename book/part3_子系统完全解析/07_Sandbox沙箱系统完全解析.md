...
- **CodeX（OpenAI）**：v0.118.0 实现了 OS 级别的网络出口限制（OS-level egress rules），取代了早期脆弱的环境变量控制。三级沙箱模式（suggest/auto-edit/full-auto）配合 Rust 重写（95.6%）带来的内存安全，与 Claude Code 的方案最为接近。
...
`shouldUseSandbox()`（`shouldUseSandbox.ts:130-153`）做出二元决策——这条命令是否要在沙箱中运行：
...
```
传统模式：  AI 决策 → 用户审批 → [沙箱] → OS 执行
                        ↑ 两道防线

autoAllow 模式：AI 决策 → [沙箱] → OS 执行
                          ↑ 一道防线
```
...
|------|----------------------|---------------------|---------------------------|
| 启动延迟 | 几乎零开销 | 数百毫秒 | 网络延迟（秒级） |
| 安装依赖 | macOS 内置；Linux 需 bwrap | 需要 Docker | 无本地依赖 |
| 隔离强度 | 进程级（seatbelt/seccomp） | 容器级（完整 namespace） | VM 级（Firecracker） |
| 文件系统 | 直接访问宿主文件系统 | 需要 volume mount 映射 | 需要文件同步 |
| 离线可用 | 是 | 是 | 否 |
| 安全审计 | Runtime 闭源 | Docker 开源 | 不可审计 |
