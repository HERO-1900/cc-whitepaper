...
- **Kimi Code**：Agent Swarm 架构下，协调器可同时管理最多 100 个并发子智能体的生命周期，在并行任务管理的规模上远超其他工具，但其任务模型面向云端集群调度而非终端用户的交互式前后台切换。
- **CodeX（OpenAI）**：v0.118.0 支持并行 Agent 工作流和邮箱通信机制，可对接 CI/CD 事件流实现无人值守修复，具备一定的后台任务能力。
...
关键细节：Claude API 的 `input_tokens` 是每轮累计值（包含所有历史上下文），而 `output_tokens` 是每轮独立值。所以输入取最新值（`latestInputTokens`），输出做累加（`cumulativeOutputTokens`）。如果都累加，输入 token 会被严重高估。
...
5 种远程任务类型对应不同的使用场景，但共享同一个 WebSocket 轮询和通知机制。
...
**问题发现过程**。源码注释记录了完整的决策背景：Anthropic 团队通过 BigQuery（BQ）分析生产遥测数据（2026-03-20），发现每个 Agent 在 500+ 轮对话时占用约 20MB RSS 内存。一个极端的"鲸鱼会话"（whale session）在 2 分钟内启动了 292 个 Agent，总内存飙升到 36.8GB。主要成本来源是 `task.messages` 字段——它为 UI 渲染持有每条消息的第二份完整拷贝（完整历史已保存在本地 `allMessages` 数组和磁盘 JSONL 文件中）。

**为什么是 50 条**。从用户角度看，任务面板的 UI 视窗通常只显示最近若干条消息，50 条足以覆盖用户回溯查看的典型深度。从内存角度看，50 条上限将每个 Agent 的 UI 消息内存从潜在的 20MB+ 降至可控范围。以 292 个 Agent 计算，50 条上限下的 UI 消息总内存约为原来的 1/10。
...
```typescript
// 源码: src/tasks/DreamTask/DreamTask.ts (第25-41行)
...
```typescript
// 源码: src/tasks/LocalMainSessionTask.ts (第270-302行)
...
```typescript
// 源码: src/tasks/stopTask.ts (第38-100行)
...
```typescript
// 源码: src/tasks/pillLabel.ts (第12-67行)
