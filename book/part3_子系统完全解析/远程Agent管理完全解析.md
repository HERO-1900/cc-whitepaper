...
> **源码版本**：Claude Code 2.1.88（泄露源码分析）
...
> - **处理结果（通知与完成）**：菜做好了，厨房把完成通知推到你的待处理队列里。如果厨房 30 分钟还没上菜（超时），你就标记这单失败，通知顾客。
...
- **CodeX（OpenAI）**：v0.118.0 支持并行 Agent 工作流和邮箱通信机制，但主要面向本地执行环境，不支持 Claude Code 级别的云端远程 Agent。**OpenClaw（原 Clawdbot）** 则代表了另一种远程执行范式——以 WhatsApp/Telegram 为入口，支持跨地域设备唤醒与远程执行，无需专用终端。
...
**Git Bundle 的意义**：引入 bundle 机制后，前置条件从"必须有 GitHub remote + App 安装"降级为"只需要有 .git 目录"。注释声称这让覆盖率从 43%（有 origin remote 的会话）跳到 54%（有 .git 的会话）。
...
`sendEventToRemoteSession()`（L361-417）通过 POST `/v1/sessions/{id}/events` 发送用户事件，超时 30 秒（CCR worker 冷启动约 2.6 秒）。
...
**855 行的单文件巨兽**：`RemoteAgentTask.tsx` 把 5 种远程任务类型（remote-agent、ultraplan、ultrareview、autofix-pr、background-pr）的完成逻辑、通知逻辑、恢复逻辑全塞在一个文件里。`startRemoteSessionPolling` 函数内部的 `poll` 闭包长达 250+ 行，嵌套了 bughunter 检测、stable idle 计算、竞态防护、TodoList 提取等多种关切。这是典型的"积累式复杂度"——每新增一种远程任务就向 `poll` 函数追加分支。
...
这些常数没有统一管理或通过 feature flag 配置（timeout 除外），调整需要改代码。
...
**pollStartedAt 重置的时间窗口问题**：恢复时 `pollStartedAt = Date.now()` 意味着一个已运行 29 分钟的 ultrareview 恢复后还有完整的 30 分钟。虽然注释解释了避免"恢复即超时"的动机，但这也意味着一个故障任务可以通过反复 resume 无限延长超时。
...
1. **GitHub App 预检（50% 放弃率）的真实动机**：表面上是优化用户体验，深层是避免容器浪费。如果创建一个 CCR 容器的成本是 $0.01-$0.10（按云端 VM 冷启动 + 运行估算），50% 的 401 失败意味着一半的容器成本打了水漂。预检的 ROI 是显而易见的。
...
3. **Fire-and-forget 归档的成本隐患**：如前所述，归档失败意味着容器不被释放。如果 CCR 没有独立的超时回收，一个"僵尸容器"可能持续计费数小时甚至数天。这使得归档可靠性从"nice to have"变为直接影响 Anthropic 运营成本的关键路径。

4. **HTTP 轮询 vs SSE 的服务端成本**：每秒一次的 HTTP 请求虽然对单用户成本极低，但在服务端产生的请求处理开销（TLS 握手、负载均衡、日志记录）远高于 SSE 长连接的心跳开销。当活跃用户增长到万级，轮询产生的请求量级（~5 万 req/s）可能驱动 Anthropic 重新评估通信方案。
