...
| BigQuery metrics | 1P API（非 Bedrock/Vertex/**Foundry**）或 C4E 或 Teams | OTel metrics | Periodic reader | 5 分钟 | — | Anthropic 内部 |
...
- **Client Token 硬编码**：`DATADOG_CLIENT_TOKEN` 直接写在源码中。这是 client token（不是 API key），安全风险有限，但泄露的源码暴露了这个值。
...
注意一个重要的哲学："谁的遥测谁控制"。OTLP 是用户自己启用的（通过 `CLAUDE_CODE_ENABLE_TELEMETRY`），所以 `DISABLE_TELEMETRY` 不影响它——用户选择导出自己的数据不受 Anthropic 隐私设置约束。而 `isAnalyticsDisabled()` 会关闭 Datadog 和 1P，因为这些是 Anthropic 消费的数据。
...
即使是 ant-only 功能，Perfetto 展示的**多 Agent 可视化思路**对从业者有极高参考价值——将 Agent 层级映射到 Chrome Trace 的进程/线程模型是一种优雅且零成本的可视化方案。
...
**降级策略**：所有 GrowthBook 查询函数都带有 `_CACHED_MAY_BE_STALE` 后缀，说明使用的是本地缓存值。当 GrowthBook 不可达时：
...
- [x] 忠实：所有行号和代码片段来自源码验证，行数统计与 wc -l 结果一致
