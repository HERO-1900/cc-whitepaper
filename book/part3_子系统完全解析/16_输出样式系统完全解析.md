...
> 
> - **Cursor**：从早期的 `.cursorrules` 单文件发展到 2025 年的 `.cursor/rules/` 多规则目录，支持 Project Rules 和 User Rules 分层，并可通过 `@rules` 在对话中动态引用特定规则——功能上已与 Claude Code 的多层样式系统高度对标。
> - **Windsurf（Codeium）**：通过 `.windsurfrules` 文件和全局 Rules 功能实现了项目级与用户级的分层配置，2025 年已相当成熟。
...
> - **ChatGPT**：2024 年推出的 Custom Instructions 和 Memory 功能率先验证了"用户持久化 AI 行为偏好"的市场需求——Claude Code 的样式系统可以看作这个方向在编程工具中的垂直化实现。
> 
> 在这个设计空间中，存在三种范式——(a) IDE 设置集成（Copilot：零成本但不灵活），(b) 纯文本规则文件（Cursor/Aider/Windsurf：简单但缺少结构化元数据），(c) 结构化样式系统（Claude Code：Markdown + Frontmatter 元数据 + 五层优先级合并）。Claude Code 的独特之处不是"样式即提示词"这个概念本身（Cursor 的规则文件本质上也是），而是**将其做成了带 frontmatter 元数据的结构化系统**，以及**五层优先级合并机制**。特别是 Learning 模式将"AI 教练"理念具体化为 CLI 环境中的 `TODO(human)` 协议——这在执行层面是新颖的，尽管苏格拉底式对话的教育理念本身已有先例（如 Khan Academy 的 Khanmigo，2023 年）。
...
| `output-style.tsx` | `src/commands/` | `/output-style` 命令（已废弃） |
| `OutputStylePicker.tsx` | `src/components/` | 样式选择器 UI |
| `loadPluginOutputStyles.ts` | `src/utils/plugins/` | 从插件加载样式 |

核心模块代码量很小——`loadOutputStylesDir.ts` 仅 99 行，`outputStyles.ts` 约 217 行。与此同时，输出样式的 prompt 会被注入到**每一次 API 调用**的系统提示词中，影响范围远超代码本身的体量。值得注意的是，代码量小并不直接等同于好设计——这也意味着缺少显式的长度限制、内容校验和安全边界检查（详见第 8 节安全分析）。
...
- `false`：**替换**默认编码指令，样式 prompt 成为系统提示中关于任务执行的唯一指导
...
> **📚 课程关联**：Learning 模式的"在决策点暂停请求人类输入"设计，映射到《软件工程》中的 **scaffolding（脚手架教学法）**和**主动学习（Active Learning）**理论——AI 搭建框架后让学习者填充关键逻辑，这与 Vygotsky 的"最近发展区（ZPD）"理论一致。从技术实现角度看，`TODO(human)` 标记本质上是一个**协程让出点（yield point）**——AI 执行流在此暂停，等待人类输入后恢复。
...
- **无长度上限**：用户理论上可以写一个 10000 token 的样式 prompt，这会显著增加每次 API 调用的成本和延迟。以一个会话内 30 次 API 调用计算，一个过长的样式 prompt 可能在不知不觉中浪费数万 token。
- **无内容过滤**：样式 prompt 中可以包含任何文本，包括试图覆盖 Claude 安全边界的指令。虽然 Claude 模型本身有安全训练来抵抗此类攻击，但将用户可控文本直接注入 system prompt 仍然扩大了攻击面。
- **无 schema 验证**：frontmatter 的字段名和值没有严格校验。错误的字段名（如 `keepCodingInstructions` 而非 `keep-coding-instructions`）会被静默忽略，不产生任何警告。
...
1. **极简核心**：加载模块仅 99 行，样式定义约 217 行——通过将复杂性下沉到提示词层面，实现了代码层面的简洁。但需要注意，这种简洁的代价是缺少运行时校验（如 prompt 长度限制、内容安全检查），安全边界依赖于用户的自觉
...
2. **插件样式冲突处理粗糙**：多个 `forceForPlugin: true` 时取第一个是一个非确定性的行为（取决于 `Object.values()` 的遍历顺序），应该有更好的冲突解决策略——如明确的优先级声明或用户选择
