...
`src/services/analytics/` 目录共 9 个文件，约 4040 行代码：

| 文件 | 行数 | 职责 |
|------|------|------|
| `index.ts` | 174 | 公共 API——logEvent/logEventAsync + 启动前事件队列 |
| `sink.ts` | 115 | 事件路由——将事件分发到 Datadog 和 1P 两条管线 |
| `metadata.ts` | ~800 | 元数据丰富——给每个事件附加设备/用户/环境信息 |
| `firstPartyEventLogger.ts` | ~400 | 第一方事件记录——OpenTelemetry Logger + 采样 |
| `firstPartyEventLoggingExporter.ts` | ~800 | 第一方事件导出器——批量发送 + 重试 + 离线缓存 |
| `datadog.ts` | ~280 | Datadog RUM 日志——事件白名单 + 批量发送 |
| `growthbook.ts` | ~1000 | GrowthBook 集成——特性门控 + A/B 实验 + 远程配置 |
| `config.ts` | 38 | 全局禁用判断——测试/第三方提供者/隐私级别 |
| `sinkKillswitch.ts` | 26 | 紧急关闭开关——独立于 config 的远程 kill-switch |
...
- **通道 ①（API Header）**：每次 API 请求都携带 attribution header，嵌入在系统提示词上下文中，无法在不影响功能的情况下单独关闭。这是最基本的使用追踪。
- **通道 ② + ③**：可通过隐私级别设置关闭（`telemetry.disabled`），但通道 ① 始终活跃。
...
> **标识符追踪链**：即使用户更换 IP、清除 cookie，以下标识符的组合仍然可以关联同一用户：`deviceId`（终身不变，存在本地 keychain）+ `accountUuid`（账号级）+ 仓库指纹（从消息的第 4/7/20 字符提取 SHA256）+ 环境指纹（20+ 种终端类型、30+ 种云平台检测）。了解这一点对做出知情的隐私选择很重要。
...
|------|---------------|-----------------|
| 运行时效果 | 编译器关闭借用检查器保护，真正改变运行时行为 | 编译后完全消失，零运行时语义 |
...
6. **全局关闭选项**：`isTelemetryDisabled()` 允许用户完全关闭
...
**"Essential Traffic Only" 的模糊性**：`isEssentialTrafficOnly()` 的具体含义在源码中不够明确——哪些流量是"essential"的？是否包括 GrowthBook 特性检查？如果包括，那么用户在设置最高隐私级别时，特性门控（如语音 kill-switch）可能无法正常工作。
...
6. **`tengu_` 前缀的遍布**：几乎所有遥测事件都以 `tengu_` 为前缀（tengu 是 Claude Code 的内部代号），这种命名在源码开源/泄露后失去了隐蔽性，反而增加了代码理解难度
