...
> **🌍 行业背景**：语音输入正在成为 AI 编程工具的差异化竞争点。**GitHub Copilot Voice**（2022年预览）是最早的尝试，基于 VS Code Speech 扩展实现语音到代码的转换，但仅支持英语且已停止更新。**Cursor** 在 0.40+ 版本中集成了语音输入（使用 Whisper API），支持 Composer 模式下的语音交互。**Aider** 通过第三方插件 `aider-voice` 支持语音，但不是官方功能。**CodeX（OpenAI）** 目前不支持语音输入。在 CLI 终端环境中实现语音输入的关键技术挑战是：终端不像浏览器那样有 Web Audio API，必须依赖外部录音工具（SoX）或平台原生模块，这也是 Claude Code 语音系统架构复杂度高于 GUI 编辑器方案的根本原因。

---

## 本章导读

语音系统是 Claude Code 2.1.88 中的一个实验性子系统，允许用户通过"按住说话"（hold-to-talk）的方式用语音输入代替键盘打字，将口述内容转换为文字后提交给 Claude。
...
默认配置是裸空格键——注意所有修饰符均为 `false`，这意味着默认触发方式是**快速连按 5 次空格**，而非修饰键组合：
...
**WARMUP_THRESHOLD 的渐进反馈设计**值得特别关注。`WARMUP_THRESHOLD = 2` 意味着用户连按 2 次空格时就会看到预热反馈（`voiceWarmingUp: true` 触发 UI 提示），而不需要等到第 5 次才有响应。这是一个**感知延迟优化**（perceived latency optimization）——用户在按到第 2 次时就知道"系统收到了，正在准备"，而不是毫无反馈地按到第 5 次。在终端环境中，由于缺乏浏览器那样丰富的视觉反馈手段（悬浮提示、动画过渡等），这种提前反馈对用户体验的影响尤为显著。
...
关键词通过 WebSocket 初始化消息以 `term:boost` 的格式传递给服务端 STT 引擎：

```typescript
// 关键词格式示例（Deepgram keywords 参数格式）
keywords: ["TypeScript:2", "npm:1.5", "git:2", "React:1.5", ...]
```

`boost` 数值控制了 STT 引擎在多个候选词之间做决策时，对该关键词的偏好程度。较高的 boost 值意味着当语音模糊时（如 "git" 和 "get" 的发音极度接近），引擎会更倾向于选择关键词列表中的词。
...
4. **`src/voice/` 目录只有一个文件**：整个 `src/voice/` 目录只包含 `voiceModeEnabled.ts`（55 行），这个目录的存在意义不大，可以合并到其他位置
