...
Agent 与任务系统是 Claude Code 的并行计算架构——当 Claude 需要"分身"时，它会创建真正独立的 AI 实例。从单个子 Agent 的生命周期管理、到 Coordinator 模式下的多 Worker 编排、到异步后台任务的进度追踪，这套系统让一个 AI 对话变成了分布式工作系统。本章将解析 Agent 创建与运行的完整链路、三种隔离模型、约 250 行 Coordinator 系统提示词、以及 DreamTask 的记忆整理机制。
...
- **Kimi Code**：实现了业界首个生产环境中的 **Agent Swarm**——协调器可动态实例化**最多 100 个隔离并发子智能体**，并行度远超其他工具。这是与 Claude Code Coordinator 在理念上最接近、但规模上差距最大的竞品方案。
- **CodeX（OpenAI）**：v0.118.0 版本引入**并行多 Agent 异步通信**和邮箱通信机制（Mailbox），支持 CI/CD 事件流的无人值守修复。底层 Rust 重写（95.6%）带来极高的并发性能。
...
baseInputSchema = z.object({
  ...
  subagent_type: z.string(),    // 专用 Agent 类型（可选）
  model: z.enum(['sonnet','opus','haiku']),  // 模型覆盖（可选）
  run_in_background: z.boolean()  // 后台运行（可选）
})
// 多 Agent 模式额外参数：name, team_name, mode
// 隔离模式：isolation: 'worktree' | 'remote'
// 工作目录覆盖：cwd（KAIROS feature gate）
```

### 1.2 runAgent 核心循环

`runAgent()`（`runAgent.ts:248+`）是一个 **AsyncGenerator**——每产出一条消息就 yield 给调用者：
...
|------|------|---------|------|
| 默认 | 共享工作目录 | 共享 | 共享 |
| `worktree` | 独立 git worktree | 隔离分支 | 共享 |
| `remote`（ant-only）| CCR 远程环境 | 完全隔离 | 隔离 |
...
> 省去这部分可以节省约 **5-15 GTok/week**（每周 50-150 亿 token）

这说明只读类子 Agent 的调用量惊人。
...
前台 Agent 运行超过 2 分钟后自动转入后台——用户不需要等待长时间任务。
...
2. **LLM 能力依赖**：Claude Code 的底层模型（Claude 3.5 Sonnet/Opus）具备足够强的指令遵循能力，使得"用自然语言定义编排规则"成为可行方案。如果底层模型的指令遵循能力较弱，这条路线就不可行。
...
3. `<task-notification>` XML 伪装为用户消息——统一了输入格式同时支持多 Agent 通信
