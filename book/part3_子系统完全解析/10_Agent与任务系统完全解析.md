...
- **Kimi Code**：实现了业界首个生产环境中的 **Agent Swarm**——协调器可动态创建（"实例化"）**最多 100 个互不干扰、同时运行（"隔离并发"）的子智能体**，并行度远超其他工具。这是与 Claude Code Coordinator 在理念上最接近、但规模上差距最大的竞品方案。
...
`runAgent()`（`runAgent.ts:248+`）是一个 **AsyncGenerator**（异步生成器——一种"边做边汇报"的编程模式，子 Agent 每完成一步就立刻把结果传回给父 Agent，而不是全部做完才一次性返回）：
