...
- **Mulberry32 PRNG**（`companion.ts` 第 16-25 行）：一个 32 位可播种伪随机数生成器，使用加法-乘法-异或移位的组合操作。注意：Mulberry32 **不是**线性同余生成器（LCG），它属于**非线性 PRNG**，更接近 SplitMix 家族——通过多轮位混合（`Math.imul` + 异或右移）来消除输入中的统计偏差，而非 LCG 那样的 `a * x + c mod m` 简单线性递推。选择 Mulberry32 而非 xoshiro256++ 或 PCG 等更现代的 PRNG，原因很直接：32 位状态对 18 种物种 × 5 种稀有度的抽取空间绰绰有余，且实现仅需 8 行代码，对 bundle size 几乎零影响。
...
一个 Legendary 伴侣的 peak 属性可以达到 `50 + 50 + 29 = 129`，但被 `Math.min(100, ...)` 截断为 100。值得注意的是，即使是 Legendary 的最弱属性（dump）下限也有 `Math.max(1, 50 - 10 + 0) = 40`——比 Common 伴侣 peak 属性上限 `Math.min(100, 5 + 50 + 29) = 84` 的一半还高。虽然当前属性是装饰性的，但这种数值设计预留了未来功能化的空间。
...
`sprites.ts` 为每个物种定义了 3 帧动画，每帧 5 行 12 字符宽。以鸭子为例（第 27-49 行）：
...
这借鉴了宝可梦的闪亮（Shiny）机制——极低概率出现特殊变体，增加收集乐趣。这是 gacha/收集类游戏的标准设计模式，宝可梦系列从1999年的金银版开始就使用基于 Trainer ID 和 Pokemon ID 异或的确定性闪亮判定。
...
3. **稀有度当前为装饰性，但数值设计预留了功能化空间**：除了视觉差异（帽子、星级、颜色），当前属性值不影响伴侣行为。但 Legendary 最弱属性下限（40）已超过 Common 最强属性上限（84）的一半，这种刻意拉大的数值梯度暗示团队可能计划在未来让属性影响伴侣的实际行为
