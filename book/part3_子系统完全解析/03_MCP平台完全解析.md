...
MCP（Model Context Protocol）是 Claude Code 的外部能力扩展层——它让 AI 不再局限于本地文件和 bash 命令，而是能连接到任意外部服务（数据库、API、浏览器、Telegram 等）。27 个 TypeScript 文件构成了一个完整的连接管理平台，支持 4 种核心传输协议（及 4 种部署变体）、8 种配置来源、手机端权限审批中继。本章将解析从配置合并到 Channel Permission Relay 的完整链路。
...
MCP 服务器配置来自 8 个不同的来源（`config.ts:69-81` `addScopeToServers()`），每个带有 `scope` 标记：
...
`unwrapCcrProxyUrl()` 从代理 URL 的 `mcp_url` 查询参数中提取原始厂商 URL——这让签名去重（signature-based dedup）可以匹配代理 URL 和直连 URL 指向同一个 MCP 服务器的情况。
...
| 类型 | 协议 | 适用场景 | 来源 |
|------|------|----------|------|
| `stdio` | 子进程 stdin/stdout | 本地 MCP 服务器（最常见） | MCP 协议最初设计 |
| `sse` | HTTP Server-Sent Events | 远程 HTTP MCP 服务器 | MCP 早期 HTTP 方案（遗留） |
| `http` | Streamable HTTP | 现代 HTTP MCP（支持流式） | MCP 后续替代方案 |
| `ws` | WebSocket | 全双工 WebSocket MCP 服务器 | 社区需求推动 |
...
`connected` 状态携带 `capabilities` + `tools` + `resources`。
...
| `xaa.ts` + `xaaIdpLogin.ts` | 企业 SSO/IdP 登录（XAA = 外部认证代理？） |
...
MCP 服务器本质上是**第三方代码**——通过 stdio 传输时，它们以子进程形式直接运行在用户机器上，拥有与用户相同的文件系统访问权限。随着 MCP 生态的快速扩张（截至 2025 年底，社区已有数千个 MCP 服务器），供应链攻击已成为最现实的安全威胁，类似于 npm/PyPI 生态曾经历的恶意包问题。

**已知的真实攻击案例：**

- **CVE-2025-6514（mcp-remote）**：`mcp-remote` 包（下载量超 43 万次）存在严重漏洞（CVSS 9.6），攻击者可通过恶意 OAuth 端点实现远程代码执行——这是首个被记录的针对 MCP 客户端的真实 RCE 案例
- **恶意 MCP 服务器窃取数据**：伪装成合法自动化工具的恶意 MCP 服务器被发现在后台转发企业邮件，其恶意行为与正常服务器操作难以区分
- **Smithery.ai 路径穿越**：MCP 服务器托管平台的构建管道存在路径穿越漏洞，攻击者可读取 3000+ 托管服务器的认证 token
...
```
plugin（最低） → user → project(approved) → local（最高）
```
