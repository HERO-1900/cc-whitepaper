...
Claude Code 的记忆系统在行业中是最复杂的实现之一：以 Auto Memory 为核心的多存储体系、自动后台提取、Dream 整合、相关性检索（Sonnet 侧查询）、团队共享记忆。这种设计的代价是系统复杂度高、API 调用成本增加，但换来了显著更强的上下文连续性。

> 📚 **学术理论视角：Tulving 记忆框架**
>
> 认知心理学家 Endel Tulving 在 1972 年提出的三类记忆系统，恰好可以映射到 Claude Code 的记忆架构上：
>
...
**后加载的优先级更高** — 模型对最后注入的内容关注度最高。这遵循了软件配置的标准"最近原则"——本地配置覆盖全局配置，与 CSS 层叠规则、Git 配置的 `--local > --global > --system` 优先级链如出一辙。

---

## 2. 核心子系统一：Auto Memory (memdir)
...
**阶段 2**：使用 Sonnet 模型（`sideQuery`，独立于主对话的 API 调用）从清单中选择最多 5 个相关记忆。选择器使用 JSON Schema 强制输出 `{ selected_memories: string[] }`，`max_tokens: 256`。
...
## 4. 核心子系统三：后台记忆提取（extractMemories）
...
**成本影响**：没有 cache 共享，每轮提取需要重新发送完整的 system prompt + 工具定义 + 对话历史作为 input tokens，成本将与主对话本身相当（Sonnet 的 input token 费用 × 完整上下文长度）。有了 cache 共享，提取调用只需为 cache 未覆盖的增量消息付费（cache 命中部分按 10% 费率计算）。这使得"每轮自动提取"从经济上不可接受变为完全可行。
...
> 📖 **延伸阅读**：本章聚焦于单用户记忆系统。当多个团队成员需要共享 AI 记忆时（如项目级知识同步），Claude Code 还有一套独立的**团队记忆同步系统**（2,167 行），通过 Anthropic API 实现 Pull/Push 同步、密钥扫描、ETag 乐观并发等。详见 **Part 3「团队记忆同步完全解析」**。
...
- 成本：每轮对话多一次 API 调用
- 单点故障：如果侧查询超时或失败，所有深层记忆都不会被召回

一个可能的改进是本地向量搜索或 embedding 缓存，但这会增加客户端复杂度。
...
`waitForSessionMemoryExtraction()` 使用 `sleep(1000)` 轮询。虽然有双超时保护，但在快速对话场景中，每秒轮询可能不够响应。使用 Promise 或 EventEmitter 会更优雅。

**4. 200 文件上限的隐含约束**

`MAX_MEMORY_FILES = 200` 是一个隐含的硬上限。对于长期使用的重度用户，这可能成为限制。系统没有提供记忆老化/归档机制（`memoryAge.ts` 只计算年龄但不自动清理），Dream 整合是唯一的整理手段。
...
Claude Code 的记忆系统并非孤立存��。以下是与主流 AI 记忆框架的架构级对比：

| 维度 | Claude Code | LangMem / Mem0 | Zep | OpenClaw |
|------|------------|----------------|-----|----------|
| **存储范式** | 纯文件系统（.md 文件 + YAML frontmatter） | 向量数据库 + 语义索引 | 知识图谱 + 向量混合 | 文件系统（类 CC 设计） |
| **召回方式** | LLM 侧查询（Sonnet 扫描 metadata → 选择相关文件） | Embedding 语义搜索 | 混合检索（语义 + 图遍历） | LLM 扫描（类 CC 模式） |
| **记忆类型** | 四类型分类学（user/feedback/project/reference） | 自由标签 | 实体关系模型 | 简化版类型系统 |
| **遗忘策略** | 粗粒度（24h/5 轮周期 + Dream 整合） | TTL + 重要性衰减 | 图修剪 + 节点权重���减 | 类 CC 模式 |
| **关联网络** | 无（扁平文件列表） | 有（向量空间近邻） | 有（知识图谱边） | 无 |
| **离线整合** | autoDream / KAIROS 夜间整合 | 无 | 后台图更新 | 无 |
