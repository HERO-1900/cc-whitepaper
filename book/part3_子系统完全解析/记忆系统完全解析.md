...
> **重要澄清**：以下五个子系统之间**不存在分层依赖关系**（不像 OSI 七层模型那样 L1 调用 L2、L3 依赖 L4）。**Auto Memory 是整个记忆系统的核心**——它拥有最精密的提取管道、类型分类学、Dream 整合和相关性检索。其余四个子系统各自独立运作，解决特定场景的存储需求：CLAUDE.md 是用户手动管理的配置层，Session Memory 是单会话压缩辅助机制，Agent Memory 是 AgentTool 的附属存储，Team Memory 只是 Auto Memory 目录下的 `team/` 子文件夹。
...
路径计算逻辑位于 `src/memdir/paths.ts`，解析顺序如下：
...
**安全加固**：`validateMemoryPath()` 函数（第 109-150 行）对路径做了 7 项安全检查：
- 拒绝相对路径
- 拒绝根目录/近根目录（长度 < 3）
- 拒绝 Windows 驱动器根目录
- 拒绝 UNC 路径
- 拒绝 null 字节（防止 C 层面的截断攻击）
