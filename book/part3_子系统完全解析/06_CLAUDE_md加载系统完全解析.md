...
这意味着企业管理员可以通过 `/etc/claude-code/CLAUDE.md` 和 `/etc/claude-code/.claude/rules/*.md` 注入**不可覆盖、不可排除**的全局策略——例如"禁止生成涉及竞争对手的代码"、"所有输出必须符合公司编码规范"。用户无法通过 `claudeMdExcludes` 配置绕过这些策略。
...
- **项目 settings 中的路径设置**：**完全不信任**用于 Memory 路径（防止 `autoMemoryDirectory: '~/.ssh'` 攻击，见 Memory 系统分析）
...
2. User CLAUDE.md + ~/rules/*.md（用户全局）
...
**关键限制**：`Managed` 和 `AutoMem` 类型**不受** `claudeMdExcludes` 影响。这意味着企业策略无法被开发者排除。
...
也就是说：`getUserContext()` 加载 CLAUDE.md 后，调用 `setCachedClaudeMdContent()` 将内容写入一个独立的缓存模块。yoloClassifier 从这个缓存读取内容，而不是直接 import claudemd.ts。这是一个经典的**依赖注入模式**——通过一个轻量的中间缓存（在 `bootstrap/state.ts` 中），打破了模块级别的循环依赖，同时保证 yoloClassifier 总能访问到最新的 CLAUDE.md 内容。
