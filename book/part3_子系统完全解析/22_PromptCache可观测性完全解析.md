...
Claude Code 每次调用 API 时都会发送 15,000-34,000 token 的系统提示词。如果这些 token 命中了 Anthropic 的 Prompt Cache，费用只有 1/10；如果没命中，就是 10 倍开销。Prompt Cache 可观测性系统（727 行）就是"侦探"——当缓存意外失效（cache break）时，它能精确定位是哪一行代码、哪一个参数的变化导致了失效，帮助开发者避免不必要的成本激增。

> **源码位置**：`src/services/api/promptCacheBreakDetection.ts`（727 行）、相关：`src/utils/tokenBudget.ts`（74 行）
...
本章分为两个部分：第一部分解析 Prompt Cache Break 检测系统（为什么缓存会失效、如何精确定位原因）；第二部分介绍 Token Budget 解析器（一个小而精的辅助系统，让用户用自然语言控制 token 消耗）。
...
## 第二部分：Token Budget 解析器（74 行）

### 7. 功能

`parseTokenBudget`（`src/utils/tokenBudget.ts`，74 行）是一个小而精的解析器——它让用户用自然语言控制 AI 的 token 消耗预算：
...
**仅 74 行实现自然语言 token 预算控制**——这是一个教科书级的"小而美"设计：
...
以下三个子系统的代码量不足以独立成章，但在 Claude Code 的运行时行为中各自扮演不可忽视的角色。本附录将它们作为"小型子系统群"进行集中解析。
...
`ComputerUseApproval` 组件是一个典型的"双面板调度器"（Two-Panel Dispatcher）。根据请求状态，它展示完全不同的 UI 面板：
...
| 维度 | 可能的值 | 对应 Header |
|------|---------|-------------|
| 基础状态 | `allowed` / `allowed_warning` / `rejected` | `anthropic-ratelimit-unified-status` |
| 超额状态 | `allowed` / `allowed_warning` / `rejected` | `anthropic-ratelimit-unified-overage-status` |
| 限流类型 | `five_hour` / `seven_day` / `seven_day_opus` / `seven_day_sonnet` | `anthropic-ratelimit-unified-representative-claim` |
| 超额禁用原因 | `out_of_credits` / `org_level_disabled_until` / ... | `anthropic-ratelimit-unified-overage-disabled-reason` |
...
**针对不同用户类型的差异化文案**：Team/Enterprise 用户看到的是"Request extra usage"（向管理员申请），Pro/Max 用户看到的是"Upgrade your plan"。Ant 内部员工还能看到 Slack 频道链接和 `/reset-limits` 命令。
...
`mockRateLimits.ts`（~400 行）实现了一个完整的限流场景模拟器，定义了 22 种 `MockScenario`：从 `normal`（无限流）到 `opus-limit`（Opus 模型专属限流）、`fast-mode-short-limit`（快速模式短时限流）等。每种场景设置一组特定的 HTTP header mock 值。
...
**信息前缀的脆弱匹配**：`RATE_LIMIT_ERROR_PREFIXES` 数组用前缀字符串匹配来判断一条消息是否是限流错误——"You've hit your"、"You've used" 等。任何文案修改都可能导致 UI 组件无法识别限流状态。更稳健的设计应该在消息对象上附加结构化的类型标签，而非依赖文本内容匹配。
...
`remoteBridgeCore.ts` 的 `initEnvLessBridgeCore()` 函数实现了完整的无环境层桥接，代码注释清晰地标注了十个步骤。第 12 章讲了前三步（创建 session → 获取凭证 → 建立传输），这里补充后七步的实现细节：
