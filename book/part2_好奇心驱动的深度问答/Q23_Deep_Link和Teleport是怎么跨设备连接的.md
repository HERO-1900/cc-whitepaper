...
> 🌍 **行业背景**：Deep Link（从浏览器唤起本地应用）是移动端的成熟技术，但在 CLI 开发工具中很少见。**VS Code** 支持 `vscode://` 协议打开文件和安装扩展，**JetBrains Toolbox** 支持 `jetbrains://` 协议打开项目，这些是桌面 IDE 中最接近的先例。**Cursor** 和 **Windsurf** 作为 VS Code 分支继承了 `vscode://` 机制，但没有针对 AI 提示词预填的 deep link 设计。至于远程代码传送（Teleport），**GitHub Codespaces** 和 **Gitpod** 通过容器化实现云端开发环境，但它们传送的是整个仓库的 Git clone，而非 Claude Code 这种"Git bundle 三级降级"的轻量策略。**Aider** 和 **CodeX** 目前没有浏览器远程控制能力（CodeX 支持并行 Agent 工作流但面向本地终端）。**OpenClaw** 通过通信应用（WhatsApp/Telegram）实现了跨地域设备唤醒与远程执行，是一种去终端化的跨设备连接方案。Claude Code 的 Deep Link + Teleport 组合的独特之处在于：它为一个**终端工具**（而非 GUI IDE）实现了跨信任边界的意图传递，这带来了额外的安全挑战——没有 OS 级的 URL 确认对话框，必须自建安全 banner。
...
Deep Link 系统有 6 个源文件、1,388 行代码，覆盖了 URI 解析、安全验证、协议注册（macOS/Linux/Windows 三平台）、终端检测（12 种终端模拟器）、shell 注入防御、以及一套完整的安全告警 banner 系统。Teleport 系统有 4 个文件、955 行代码，实现了 Git bundle 三级降级策略、云端会话管理、环境选择，以及带重试的 API 通信层。这两个系统共同解决一个核心问题：**怎样安全地跨越信任边界传递意图**。
...
> 📚 **课程关联**：三平台的协议注册是操作系统课程中**进程间通信（IPC）**和**注册表/服务发现**机制的实际应用。macOS 的 LaunchServices、Linux 的 XDG MIME 系统、Windows 的注册表，分别代表了三种不同的"服务注册与发现"范式。符号链接（symlink）是操作系统文件系统课程的基础概念——这里巧妙地利用了 symlink 的"签名继承"特性来绕过代码签名检查。
...
`terminalLauncher.ts`（557 行，最大的单个文件）适配了 12 种终端模拟器：
...
- 如果 cwd 通过 `?repo=` 解析，banner 还会显示上次 `git fetch` 的时间——如果超过 7 天未 fetch，追加 "CLAUDE.md may be stale" 警告。因为 `CLAUDE.md` 可能包含项目级指令，stale 的 CLAUDE.md 可能缺少新的安全规则
- 路径显示使用 `tildify()`——`/Users/hero/projects` 变成 `~/projects`，让用户一眼看出工作目录
...
Teleport 是**出境传送门**：把城市里的"东西"（代码）传送到另一个平行世界（云端沙箱）。传送前先把所有行李打包（Git bundle），如果行李太重就逐步精简——先扔掉旁支（HEAD-only），再扔掉历史（squashed-root），只保留当前状态的快照。传送门还会把你正在编辑但还没保存的东西（WIP stash）悄悄塞进行李——不打扰你的工作，但确保另一边的 Claude 看到的是最新状态。

两个传送门共同依赖城市的身份认证系统（OAuth）——没有有效身份证（access token），哪个传送门都不让你用。
...
- `src/utils/deepLink/terminalLauncher.ts`：12 种终端模拟器适配
- `src/utils/deepLink/banner.ts`：安全告警 banner 系统
- `src/utils/claudeInChrome/` — Chrome 浏览器扩展集成相关
...
Deep Link 和 Teleport 解决的核心问题是**跨信任边界传递意图**——一个来自浏览器的 URL 必须经过 5 层安全检查才能变成终端里的预填提示词，一个本地仓库要经过 3 级降级打包才能安全地出现在云端沙箱里。每一层"传送"都不是简单的数据搬运，而是**在保证安全的前提下尽量不打扰用户**的精密工程。
