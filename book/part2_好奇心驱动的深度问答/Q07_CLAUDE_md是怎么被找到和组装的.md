...
对于项目级规则，系统做了一件有趣的事：**从当前目录向上遍历到文件系统根目录**，收集所有中间目录，然后反转顺序，从根往下加载。
...
> 📚 **课程关联**：这种"从根目录向下逐层覆盖"的配置发现模式，在《软件工程》和《操作系统》课程中都有对应概念。它与 Git 的配置系统（`/etc/gitconfig` → `~/.gitconfig` → `.git/config`）、npm 的 `package.json` 向上查找、以及 Linux 的 `/etc/profile` → `~/.bashrc` → `.bashrc` 分层加载机制同属一种设计模式——**分层配置覆盖**（Layered Configuration Override）。在分布式系统中，这种模式也出现在 Kubernetes 的 ConfigMap 层级覆盖和 Spring Boot 的 profile-based 配置中。
...
系统使用 `marked` lexer 来精确识别 HTML 注释（保留代码块内的注释不影响）。
...
**渐进式权限**：Local（私有）> Project（共享）> User（全局）> Managed（企业）的层次，允许"尊重团队规范，同时保留个人自定义"的使用模式。
...
- [x] 批判：指出 memoize 的局限（session 内不感知文件变更）
