...
> 🌍 **行业背景**：远程操控终端 AI 并非 Claude Code 独创——这是 2024-2025 年 AI 编程工具的共同方向。**Cursor** 的 Remote SSH 模式允许用户通过 VS Code Remote SSH 在远端服务器运行 AI 辅助编码，但本质上复用了 VS Code 的远程架构，没有独立的会话管理层。**GitHub Copilot** 在 VS Code 中通过 Codespaces 实现云端编码环境，但以 IDE 为中心而非终端。**Windsurf**（Codeium）同样依赖 VS Code 远程扩展体系。**CodeX（OpenAI）** v0.118.0 新发布了专用 macOS 独立客户端，但主要面向本地终端运行。**OpenClaw（原 Clawdbot）** 代表了另一种远程控制范式——以 WhatsApp/Telegram 等通信应用为主要交互入口，支持跨地域设备唤醒与远程执行，开发者可通过简单的手机消息触发本地电脑的复杂构建任务。Claude Code 的 Bridge 系统独特之处在于：它不依赖 IDE 远程框架，而是构建了一个从零实现的**分布式会话调度层**——包含环境注册、工作分派、JWT 认证刷新和 git worktree 隔离，更接近 Kubernetes 的编排理念而非简单的 SSH 隧道。
...
Remote Control 是一个完整的**分布式会话管理平台**，有环境注册、工作分派、JWT 认证刷新、多会话 worktree 隔离、断线重连、权限桥接——复杂度远超一个简单的终端代理。它的代码在 `bridge/` 目录下有 30+ 个 TypeScript 文件，主逻辑文件 `replBridge.ts` 单文件 2400+ 行。
...
> 📚 **课程关联**：Spawn 模式的三种隔离策略直接对应**操作系统**课程中的进程隔离模型——`single-session` 类似单进程独占资源、`worktree` 类似容器级隔离（namespace + cgroup）、`same-dir` 类似共享内存的线程模型。理解这三种模式的取舍，就是理解 OS 中"隔离性 vs 性能"这一经典权衡。
...
worktree 模式在底层调用 `createAgentWorktree()` 创建临时分支，会话结束后调用 `removeAgentWorktree()` 清理。这意味着每个从 claude.ai 发起的会话都在自己的沙箱里工作——写坏了也不影响主分支。
...
系统内部同时运行着两套连接协议，由 GrowthBook feature flag 控制切换：
...
5. **优雅关闭 ≈ Graceful shutdown**。SIGTERM 后给 30 秒 grace period，超时强杀。
...
- **笔记本休眠场景脆弱**：虽然有睡眠检测和指数退避，但长时间休眠（如过夜合盖）后的重连可能失败，用户需要手动重启 bridge
...
*质量自检：*
- [x] 覆盖：30+ 文件中的核心 12 个文件已分析，架构全景清晰
