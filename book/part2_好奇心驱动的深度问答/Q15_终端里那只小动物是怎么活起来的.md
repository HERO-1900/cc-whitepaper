...
Claude Code 的终端里住着一只 ASCII 小动物——它有确定性的基因、不可作弊的稀有度、15 步空闲动画和由 AI 驱动的实时评论。这个叫做 Buddy（代码中称 Companion）的系统，是一个完整的虚拟宠物引擎，融合了电子宠物、Gacha 游戏和终端渲染的设计理念。本章从 PRNG 种子到 ASCII 精灵动画，完整拆解这个"活"在终端里的小生命。
...
在开发工具中嵌入虚拟宠物/吉祥物是一个历史悠久但近年来复兴的传统。**微软 Office 的 Clippy**（1997-2007）是最早的 AI 助手吉祥物，但因过度打扰用户而臭名昭著——它的失败让业界对"拟人化 AI 助手"保持警惕长达十年。**GitHub** 的 Octocat 是纯视觉吉祥物，没有交互行为。**Rust 语言**的 Ferris（螃蟹）在编译器错误信息中偶尔出现，增添了人情味。在 AI 编程工具领域，**Cursor** 和 **Windsurf** 都没有虚拟宠物系统——它们聚焦于纯功能性体验。**Warp 终端**曾实验性地加入了 AI 对话的表情反馈，但远没有达到完整的虚拟宠物引擎的程度。
...
- **Bones（骨骼）**：物种、稀有度、眼睛、帽子、闪光状态、五维属性值。完全由 `hash(userId + SALT)` 确定性计算，**永不存储**。
...
1. **防作弊**。配置文件是一个 JSON，用户可以随意编辑。如果 rarity 存在配置里，改成 `"legendary"` 就行了。但 bones 从不持久化——每次读取时从 userId 重新计算。你可以改名字，但改不了命。
...
注意 `...bones` 在 `...stored` 之后——如果旧配置里残留了过时的 bones 字段，会被覆盖。
...
> 📚 **课程关联**：伪随机数生成器（PRNG）是**计算机组成原理**和**密码学**课程的核心内容。Mulberry32 属于线性同余生成器（LCG）的变体——用位运算和乘法实现状态转移，周期为 2^32。在密码学课程中会强调：PRNG 不能用于安全场景（如密钥生成），但对于游戏抽卡、UI 动画等非安全场景完全足够。这里"可种子化"的特性保证了确定性——相同输入永远产生相同输出，这是**函数式编程**中纯函数（pure function）的核心要求。

种子的来源是 `hash(userId + SALT)`。`SALT` 的值是 `'friend-2026-401'`——暗示这个功能在 2026 年 4 月 1 日（愚人节）上线。加盐的目的：即使知道算法，也无法通过构造特定 userId 来"刷"出 legendary。
...
一个有趣的规则：只有 uncommon 及以上稀有度才有帽子（`companion.ts:97`）。Common 的宠物光头出场。
...
18 种物种各有 3 帧 ASCII 精灵图，每帧 5 行 × 12 字符宽：
...
export function isBuddyTeaserWindow(): boolean {
  if ("external" === 'ant') return true  // 内部员工绕过
  const d = new Date()
...
*质量自检：*
- [x] 覆盖：6 个源文件完整分析，缺失的 observer.ts 已标注
- [x] 忠实：所有常量值、行号均来自源代码
- [x] 可读：用电子宠物/Gacha 类比建立直觉
- [x] 一致：遵循 Q&A 章节标准结构
- [x] 批判：指出了 observer 成本、ASCII 可达性、硬编码精灵图等局限
