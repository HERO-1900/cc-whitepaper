...
`processUserInput/` 目录有 4 个文件、1,765 行代码，实现了一条**多路分发管道**：图片缩放、剪贴板内容处理、IDE 选择器提取、附件加载、斜杠命令解析、bash 命令执行、子 Agent @mention 检测、ultraplan 魔法关键词、桥接安全命令过滤、Hook 拦截、权限模式注入、isMeta 隐形消息——每一步都可能改变管道的走向或终止处理。这不是一条直线流水线，而是一个充满条件分支和短路出口的**决策迷宫**。
...
2. 将输入包装为 `<bash-input>` XML 标签
3. 调用 `BashTool.call()` 或 `PowerShellTool.call()`（PowerShell 工具是**懒加载**的，`require()` 只在实际使用时触发，避免 ~300KB 的加载开销）
...
附件消息中有一种特殊类型：`agent_mention`（`processUserInput.ts:557-574`）。当用户在输入中 @mentions 一个子 Agent（如 `@agent-commit`），系统会记录这个行为用于分析——区分"只输入了 @agent 没说别的"和"@agent 后面跟了指令"两种模式。
...
Hook 输出有截断保护——`MAX_HOOK_OUTPUT_LENGTH = 10000` 字符（`processUserInput.ts:274`）。超长输出被截断并附加 "output truncated" 标记。这防止了一个行为不良的 hook 脚本用巨量输出耗尽上下文窗口。
...
**为什么 processSlashCommand 这么大（几千行）？** 因为斜杠命令系统承载了太多变体：内置命令、插件命令、Skills、可 fork 的命令、异步后台命令、MCP 等待逻辑、权限检查……每种变体都有自己的执行路径和异常处理。这是一个典型的"入口简单、内部复杂"的模块。

---

## 代码落点

- `src/utils/processUserInput/processUserInput.ts`，第 85-270 行：管道入口（18 个参数）
...
- **管道复杂度高**：18 个输入参数、6 个短路出口使得调试和测试异常困难，新开发者理解完整流程的学习曲线陡峭
...
用户输入不是"发给 API 的字符串"——它是一条经过图片缩放、附件提取、三路分发（bash/斜杠命令/文本）、安全过滤（桥接命令、魔法关键词）、和 Hook 拦截的**多阶段处理管道**。这条管道有 18 个输入参数、5 种输出消息类型、至少 6 个短路出口。它证明了一个设计事实：**在 AI 产品中，"理解用户在说什么"这一步的复杂度，往往不亚于 AI 本身的推理过程**。
