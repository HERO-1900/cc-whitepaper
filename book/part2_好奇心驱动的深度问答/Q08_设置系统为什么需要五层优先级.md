...
> 📚 **课程关联**：配置合并策略是《软件工程》课程中"配置管理"（Configuration Management）章节的核心话题。"对象深合并 + 数组替换"的选择对应两种经典的合并语义——**递归合并**（recursive merge，类似 Git 的三路合并）和**覆盖式替换**（override）。在《分布式系统》课程中，这也与状态复制的冲突解决策略相关：CRDT（Conflict-free Replicated Data Types）中的 G-Counter 采用合并语义，而 LWW-Register（Last-Writer-Wins）采用覆盖语义——Claude Code 的数组替换本质上就是 LWW 策略。

设置合并用 lodash `mergeWith`，规则是：
...
// 合并结果
{"allowedTools": ["Read", "Grep"]}  ← 完全替换
```

**重要含义**：如果企业策略设置了任何数组类型的字段，它会**完全覆盖**所有低优先级来源的同类字段。管理员可以用数组来精确控制可用工具列表，用户无法往里面"偷塞"额外工具。
...
- `src/utils/settings/settings.ts`，第 65 行：`loadManagedFileSettings()` — drop-in 目录实现

---

## 设计的代价与权衡

然而，五层配置也带来了复杂性。当用户发现某个设置"不生效"时，问题可能出在任何一层——排查需要逐层检查，这对普通用户来说是一个不小的认知负担。数组"替换"而非"合并"的语义虽然让管理员能精确控制，但也意味着用户在 `userSettings` 中添加的允许规则可能被项目级的数组完全覆盖。如果配置层级的合并逻辑出现 bug，影响范围可能是全局性的——因为所有功能都依赖这套设置系统。这种"统一基础设施"的风险在于单点失败的代价极高。
