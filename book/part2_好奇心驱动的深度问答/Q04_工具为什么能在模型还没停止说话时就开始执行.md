...
- **CodeX（OpenAI）**：v0.118.0 版本引入并行 Agent 工作流和邮箱通信机制（Mailbox），支持多后台进程异步交互。底层已用 Rust 彻底重写（95.6%），在并发性能上有先天优势，但其并行化更侧重于多 Agent 间的异步通信，而非单个 Agent 内的流式提前执行。
...
5. 调用 `getRemainingResults()` 收集所有结果，按原始顺序 yield

**关键细节：结果永远按照工具调用的顺序 yield。** 即使工具 B 比工具 A 更早完成，结果也先 yield A、再 yield B。这保证了消息历史的一致性。
...
> 📚 **课程关联 · 计算机体系结构**：这正是体系结构课程中 **CPU 流水线**（instruction pipeline）思想的软件实现。经典 5 级流水线（IF→ID→EX→MEM→WB）让多条指令在不同阶段重叠执行，把吞吐量提升数倍。StreamingToolExecutor 的设计完全对应：模型输出是"取指"（IF），JSON 解析是"译码"（ID），工具执行是"执行"（EX）——多个工具调用在不同阶段同时推进。`siblingAbortController`（一个出错则终止兄弟）则类似于流水线中的 **冲刷**（pipeline flush）——当分支预测失败时，清空已进入流水线的后续指令。
...
**按并发安全性分批：**
```
假设 AI 调用了：Read, Grep, Read, Edit, Bash
→ 批次 1（并发）：Read, Grep, Read   ← 只读，可以一起跑
→ 批次 2（串行）：Edit               ← 写操作，单独跑
→ 批次 3（串行）：Bash               ← 写操作，单独跑
```

每个工具通过 `isConcurrencySafe(input)` 方法声明自己是否支持并发。只读工具返回 true，写操作工具返回 false。系统最多同时运行 10 个并发工具（可通过环境变量 `CLAUDE_CODE_MAX_TOOL_USE_CONCURRENCY` 调整）。
...
当多个 Bash 工具在并发批次中运行（比如 AI 同时运行了三个 Bash 命令），如果其中一个出错，`StreamingToolExecutor` 会立即通过 `siblingAbortController` 中止其他正在运行的工具。
...
**优势很明显：** 工具执行的延迟从"模型输出时间 + 所有工具串行时间"变成"模型输出时间"（工具执行与模型输出重叠），对 I/O 密集的工作流有显著加速。
...
- `src/tools/StreamingToolExecutor.ts`：完整实现，关注 `addTool()`、`getCompletedResults()`、`siblingAbortController`
- `src/utils/processToolCallGroup.ts`：工具调用分组与批次执行逻辑
- `src/services/tools/toolOrchestration.ts`，`partitionToolCalls()` 函数（第 90 行起）：批次划分逻辑
- `src/services/tools/toolOrchestration.ts`，`getMaxToolUseConcurrency()`（第 7 行）：最大并发数配置
