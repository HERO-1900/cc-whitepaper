...
WebSocket 连接到 Anthropic 的 `voice_stream` 端点（底层走 claude.ai 的 conversation_engine 基础设施），使用 Deepgram 作为 STT 引擎。转录结果分两种：
...
> 📚 **课程关联**：silent-drop replay 机制是**分布式系统**课程中"客户端重试与幂等性"的实战案例。当 session-sticky 路由将请求粘在故障 pod 上时，客户端通过新建连接绕过粘性路由——这与微服务架构中的"circuit breaker"（断路器）模式异曲同工。2MB 音频缓冲则是典型的"重试缓冲区"设计，在消息队列（Kafka）和流处理系统中广泛使用。
...
### 小节 7：Focus 模式——终端获得焦点就自动录音

除了手动按键的标准模式，还有一个 Focus 模式（通过 `focusTriggeredRef`、`useTerminalFocus` 等引用可以推断）——当终端窗口获得焦点时自动开始录音。这个模式有自己的超时参数：
...
Voice 模式的设计体现了**"知道什么不做"比"做什么"更重要**的工程哲学：

1. **不做 VAD**。终端没有回声消除，VAD 在这个环境下不可靠。对讲机模式把复杂的音频信号处理问题简化为一个二值信号：按着=录，松手=停。
2. **不做 TTS**。系统只支持语音转文字（STT），不支持文字转语音（TTS）。这不是技术限制——而是产品判断：开发者需要的是快速输入想法，不是听 AI 念代码。终端本身就是最好的文字输出界面。
3. **不做自研 STT**。直接使用 Deepgram（通过 Anthropic 的 voice_stream 端点代理），不重复造轮子。Anthropic 的核心竞争力是 LLM，不是语音识别。
