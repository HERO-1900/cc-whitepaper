...
> 📚 **如果你学过操作系统**：Claude Code 的架构和操作系统有惊人的相似——它有自己的"引导序列"（启动流程）、"系统调用表"（43个工具）、"进程调度器"（Agent编排）、"权限系统"（九步检查链）、"设备驱动接口"（MCP协议）。本章用5分钟给你一个全局视图，后面的 Part 2 会逐一深入每个子系统。
...
用户输入 `/commit`、`/help` 等斜杠命令时触发。不同于工具（工具由 AI 调用），命令由**用户直接调用**，有独立的处理路径。系统有约 40 个内置命令。
...
- `src/services/claude.ts`：查询循环核心——`query()` 函数驱动 Agent Loop
- `src/tools/tools.ts`：工具注册表——38 个内置工具的统一注册
- `src/utils/permissions/permissions.ts`：权限系统——9 步状态机的完整实现
