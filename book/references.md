...
在 Claude Code 2.1.88 源码泄露事件（2026年3月31日）前后，中文技术社区爆发了一场前所未有的深度分析热潮。以下按首次引用顺序列出在本书中被直接引用或为本书提供重要启发的社区成员。
...
| **@servasyy_ai** | X/Twitter | Harness Engineering 框架（泄露前2天提出）；15章912行完整源码分析（cache_edits机制、三通道遥测追踪链、四层递进Compact、Flush Gate状态机——中文社区最全面的单篇技术分析）；CC vs OpenClaw Hooks哲学对比 | 698赞 + 404赞, 113K浏览 |
...
| **@MaxForAI** | X/Twitter | 《从Claude Code泄漏的源码，看第一梯队AI Agent工程架构》，7张架构配图 | 399赞，138K浏览 |
...
| **@GoSailGlobal** | X/Twitter | AgentSkillsHub Top 10 榜单统计，泄露后生态爆发的定量证据 | 51赞 |
...
| **@fried_rice** | X/Twitter | 最早发现并公开源码泄露事件 | 事件首发 |

---

## 开源项目

Claude Code 源码泄露催生了一场开源生态的集中爆发。泄露后一周内，GitHub Agent Skills 趋势榜前 10 中有 6 个是 CC 相关项目。以下项目在本书中被提及或为本书提供了参考。
...
| **open-agent-sdk** | — | @idoubicc | MIT 开源，函数调用式 SDK 替代方案 | 序章、系统��价（进程架构瓶颈佐证） |
...
| **GLM / Z.ai / Z Code** | 智谱 AI 编程平台 | 竞品对比（GLM-5.1 744B参数、国产Ascend 910B芯片、私有化部署） |
| **Google Antigravity** | Google 的 AI 编程工具 | 竞品对比（Mission Control 架构、Artifacts 机制、Allow/Deny List） |
...
这本书的诞生，源于一次意外的源码泄露，但它能走到今天，靠的是整个社区的集体智慧。

**感谢 Anthropic 工程团队。** 我们对你们的工程能力怀有真诚的敬意。Claude Code 是一个设计精良的系统，很多决策在深入分析后让人由衷佩服。本书中的批评同样出于真诚——指出设计取舍和潜在问题不是贬低，而是工程分析应有的态度。

**感谢社区分析者。** @tvytlx 的924K浏览深度报告、@AlchainHust 的75页橙皮书、@Barret_China 的Tulving框架映射、@0xJoooe 的Prompt工程六原则、@servasyy_ai 在泄露前两天提出的Harness Engineering框架、@yq_acc 的系统性效应论、@troyhua 的7层记忆拆解——你们的工作为中文技术社区树立了深度分析的标杆。没有这些先行者的探索，本书很多章节的分析维度会窄得多。

**感谢开源建设者。** @idoubicc 把泄露变成了开源契机，@wquguru 用两本书建立了对比框架，@jiayuan_jy 将协作需求落地为产品，ccunpacked.dev 用交互动画降低了理解门槛。你们证明了：一次源码泄露，可以成为整个行业的学习资源。
...
**感谢每一位读者。** 这本书有83章、逾36万字，能读到这里的你，本身就是对深度技术内容最好的投票。
