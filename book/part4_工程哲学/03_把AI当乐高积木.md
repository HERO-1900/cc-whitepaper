...
> 🌍 **行业背景**："一个通用执行引擎 + 参数化配置"的架构模式在软件工程中由来已久——GoF 设计模式中的策略模式（Strategy Pattern）和参数化工厂模式就是其经典表达。在 AI 框架领域，不同产品在"多种 AI 调用场景"这个相同问题上做出了截然不同的架构选择：
>
...
注意那句"Terse command-style prompts produce shallow, generic work"（"简短命令式提示词只会产生浅薄的泛化输出"）——这是 Anthropic 工程师通过实验观察到的规律，被编码进了工具描述的教学内容，对每次使用 AgentTool 的 Claude 实例生效。
...
- `src/services/claude.ts`：统一的 `query()` 函数——所有 7 种 AI 调用共用的执行入口
