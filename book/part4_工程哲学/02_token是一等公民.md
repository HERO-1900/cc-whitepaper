...
在 Claude Code 的代码库里，token 消耗是一个被反复考量的设计约束，和内存、CPU 一样被当作"一等公民"对待。
...
> 有一次尝试用 `effort:'low'` 节省预测成本——结果缓存命中率从 92.7% 降到 61%，缓存写入量激增 45 倍。
...
更进一步，Claude Code 建立了完整的缓存失效检测系统（`promptCacheBreakDetection.ts`），跟踪超过 12 个可能导致缓存失效的参数维度：system prompt hash、tool schemas hash、model、fast mode、cache_control、betas、effort、extra body params 等。每次 API 调用后，系统对比缓存读取 token 的变化——如果下降超过 5% 且绝对值超过 2000 token，就触发告警，并自动生成 diff 文件帮助诊断。

这种主动监控的做法在 LLM 应用中是不常见的——大多数工具只在账单上发现缓存问题，而 Claude Code 在每次 API 调用后都进行实时检测。

> 📚 **课程关联**：prompt cache 的缓存键问题与**数据库系统**课程中的**查询缓存失效**高度类似——MySQL 的 Query Cache 曾因为查询文本的任何微小差异（包括大小写、空格）导致缓存未命中。同样的教训：缓存键的粒度往往比你预期的更细。命中率从 92.7% 降到 61%，意味着未命中率从 7.3% 升到 39%（5.3 倍增长）。按 Anthropic 的缓存定价（命中 0.1x、写入 1.25x、正常输入 1x），混合成本从约 0.17x 升到约 0.45x——接近 3 倍的成本增长。
...
Claude Code 的六套机制逐步升级：先只丢弃工具结果的细节（snip），然后用 AI 压缩工具结果，然后折叠旧消息（context collapse），然后自动压缩（autocompact），最后完整摘要。触发阈值由 `getAutoCompactThreshold()` 动态计算：`有效上下文窗口大小 - 13000 token 缓冲`（`AUTOCOMPACT_BUFFER_TOKENS = 13_000`），其中有效上下文窗口 = 模型上下文窗口 - 输出预留（最多 20000 token）。
...
更根本的是，从 2023 年到 2025 年，主流 LLM 的 token 价格已经下降了 10-50 倍。**这不是假设，这是正在发生的趋势**。Claude Code 今天精心设计的六级压缩机制和缓存对齐策略，在未来可能变成维护负担。但也正因如此，理解哪些优化的 ROI 最高（缓存层 > 裁剪层 > 压缩层）才更重要——价格下降时，低 ROI 的优化应该首先被简化。
