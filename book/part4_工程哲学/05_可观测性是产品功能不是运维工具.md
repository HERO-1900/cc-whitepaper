...
当一个 hook 运行时，系统会产生三种事件：

```typescript
emitHookStarted(hookId, hookName, hookEvent)    // 开始
emitHookProgress(...)                            // 进度（每秒一次）
emitHookResponse(...)                            // 完成（含 stdout/stderr/exitCode/outcome）
```
...
这个 `MAX_PARENT_UNCACHED_TOKENS = 10_000` 的阈值不是拍脑袋决定的。它来自生产数据：通过追踪预测命中率 vs 父请求的缓存状态，工程师发现了"缓存冷启动时预测没有意义"的规律，然后把这个规律编码成运行时决策。值得注意的是，这个阈值是硬编码常量——当模型升级（上下文窗口变大、缓存策略变化）后可能需要重新校准。源码中没有记录这个数值的推导过程（比如是通过线性回归发现的拐点，还是散点图人工判断的），它本质上是一个带注释的 magic number，背后的"数据驱动"过程对读者来说是不可见的。
...
源码中的 BQ（BigQuery）分析注释记录了完整的调查链路：
...
工程团队没有停留在"平均内存偏高"的统计结论上。通过 BigQuery 中按 session 分组的 RSS 内存分布，他们发现了极端异常值——session `9a990de8`。这个 session 的行为特征非常明确：**2 分钟内启动了 292 个 agent**，总 RSS 达到 36.8GB。团队给这类极端会话起了一个内部术语——**"鲸鱼会话"（whale session）**。命名本身就说明异常检测已经成熟到形成了内部分类体系：不是"某个用户报了个 bug"，而是"我们有一个标准类别叫 whale session，定义是……"。
