...
> 🌍 **行业背景**：多层安全防御在信息安全领域有一个经典名称——**纵深防御（Defense in Depth）**，最早由美国国家安全局（NSA）在 2000 年代初正式提出，但其思想可追溯到军事领域的多层阵地防御。在软件安全中，OWASP 的安全设计原则、微软的 SDL（Security Development Lifecycle）都将纵深防御列为核心原则。在 AI 编码助手领域：**Cursor** 采用沙箱加用户确认的模型，且 2025 年的 Agent 模式已引入工具级权限审批（包括 yolo 模式的 allowlist/blocklist）；**Aider** 在安全方面较为薄弱，主要依赖用户手动审查 diff；**GitHub Copilot** 因为只做代码补全（不执行命令），攻击面从根本上小得多——不是"安全模型更轻量"，而是根本不需要这六层中的大部分。Claude Code 选择了"更强大但更危险"的路线——它能执行任意 Bash 命令、读写文件系统——这个根本性的能力-风险 trade-off 解释了为什么它需要比同行更多的防线。这不是 Claude Code 发明了纵深防御，而是 AI Agent 的能力范围（尤其是代码执行权限）让纵深防御从"推荐做法"变成了"生存必需"。
...
企业 MDM 或本地配置文件可以在系统级别声明"某类工具永远不允许"。这一层在其他所有判断之前执行，不可绕过。对应源码中 `hasPermissionsToUseToolInner()` 的步骤 1a——整个工具被 deny 规则拦截。

**层次 2：工具自检（checkPermissions）**

每个工具都有自己的 `checkPermissions()` 方法。Bash 工具会通过模式匹配（逐字符比对已知危险指令）检查命令是否包含已知危险模式（如 `rm -rf`（强制递归删除所有文件——一条命令就能清空整个硬盘）、`chmod 777`（把文件权限完全开放，任何人都能读写执行）等，见 `bashPermissions.ts`）；Edit 工具会检查路径是否在 `.git/`、`.claude/`、`.vscode/`、`.idea/` 等敏感目录里，或者是否匹配 `.bashrc`、`.gitconfig` 等危险文件名（`filesystem.ts` 中的 `DANGEROUS_DIRECTORIES` 和 `DANGEROUS_FILES` 列表）。
...
> 💡 **通俗理解**：这更像机场安检的两种通道——有人值守的普通通道（用户确认）和自助快速通道（AI 分类器）。两种通道不会同时走，但无论走哪条，前面的 X 光机（工具自检）和后面的空警（bypass-immune）都在工作。
...
bypass-immune 是整个防御模型中最有工程智慧的部分，值得深入理解。
...
**前四层（策略/自检/Safety Check/规则匹配）几乎无开销**。它们全部是内存中的字符串匹配和规则查表，执行时间在微秒量级。
...
| 层 | 开发时间估算 | 运行时开销 | 维护复杂度 |
|---|------------|----------|----------|
| 工具自检 | 1-2 天 | 几乎为零 | 低（更新危险模式列表） |
| bypass-immune | 半天 | 几乎为零 | 低（列表很少变化） |
| 用户确认 | 1-3 天 | 无（等待用户输入） | 低 |
| 企业策略 | 1-2 周 | 几乎为零 | 中（策略管理界面） |
| AI 分类器 | 2-4 周 | 高（每次调用一次 LLM） | 高（prompt 调优、误判处理） |
| Hooks 系统 | 1-2 周 | 取决于 hook 实现 | 中（需要文档和示例） |
