...
Token 消耗是 Claude Code 的核心成本约束——每次 API 调用都涉及输入 token 计费、Prompt Cache 命中优化和输出 token 预算控制。本章解析系统如何通过六层上下文压缩、缓存友好的提示词排列和投机执行来管理这个"货币系统"。

> **源码位置**：`src/utils/token.ts`（计数与预算）、`src/utils/compact.ts`（上下文压缩）、`src/services/claude.ts`（API 调用与缓存参数）
...
| **GLM（Z.ai）** | 7440 亿参数 + 国产 Ascend 910B 芯片 | 权重开源 / 订阅制 | 全量训练在国产芯片上完成；Z Code 平台面向私有化部署，数据不出境 |
...
Claude Code 的 Token 就是这座城市的"水"。每次 API 调用消耗 token，每个 token 都有成本。系统中大量设计决策——上下文压缩、Prompt Cache、投机执行、工具描述延迟加载——本质上都是在做**水资源管理**：减少浪费、复用已处理的水、在低谷时储水。正如一位社区开发者所概括的："上下文是最贵的资源，必须主动管理。"
...
**数字：** 一个典型的对话，系统提示词 + 工具 Schema 大约占 20,000-40,000 token。如果每次都命中缓存，这些 token 的成本降到 1/10——每次 API 调用节省 $0.3-0.6。一个 20 轮的对话节省 $6-12。

### 2.4 CacheSafeParams

`CacheSafeParams` 是一个类型约束——保证特定的参数组合不会破坏 Prompt Cache。
...
> 💡 **通俗理解**：就像图书馆的书架——你不需要把整排书搬走再重新排列（缓存失效），���需要在不想要的书上贴一个"跳过"标签（cache_edits），管理员扫描时直接跳过这本书，其他书的位置完全不变。
...
上表只是冰山一角。`promptCacheBreakDetection.ts`（700+ 行）实际追踪了 **14 个不同的缓存失效向量**，包括模式切换、工具增删、CLAUDE.md 变更等。系统还使用了"粘性闩锁"（sticky latch）机制——一旦进入某个模式就尽量不切换回来，因为模式切换几乎必然导致缓存前缀不匹配。这不再是"性能调优"——在规模化场景下（每日数百万次 API 调用），每次缓存未命中都是实际的成本支出，Prompt Cache 本质上是一个**计费优化问题**。
...
如果不压缩，大约 30-50 轮对话后就会达到 `blocking_limit` ——心脏停跳，对话被迫终止。
...
每个工具有 `maxResultSizeChars` 限制。超过的部分被截断。这不是为了安全——是为了**控制 input token 量**。一个 Read 工具读取 10,000 行文件，全部放进上下文就是几万 token。系统设置合理的上限，平衡"AI 需要看到足够信息"和"不浪费 token"。
...
**Effort 系统的经济含义**：Opus 4.6 Pro 的默认 effort 是 `medium`（而不是 `high`）——不是因为 medium 质量够好，而是因为**最大化 rate limits**。rate-limited 的 Pro 用户如果每次都用 high effort，会很快打到限制。medium 是性能/成本/配额的最优平衡点。
...
团队估算的 **5-15 GTok/week**（每周 50-150 亿 token）说明 Agent 调用的量级是惊人的——Claude Code 的 Agent 子系统不是偶尔使用的功能，而是**核心工作路径**。
...
**Cursor** 走了另一条路：不依赖单一的缓存机制，而是通过**模型分级**来控制成本。Tab 补全用 GPT-3.5 级别的小模型（几乎不花钱），复杂任务才调用 GPT-4/Claude（贵但强）。这相当于"把便宜的活交给便宜的工人"。
