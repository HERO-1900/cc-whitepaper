...
**关键细节**：这里利用了 **Bun bundler 的顺序求值语义**——每条 `import` 会先完成该模块的加载和求值，然后才执行下一条语句。因此代码的实际执行顺序是：导入 `profileCheckpoint` → 调用它 → 导入 `startMdmRawRead` → 调用它 → 导入 `startKeychainPrefetch` → 调用它。每个函数只依赖自己那一条 import，不需要等后续模块加载完成。这比"在所有 import 之前"更精确——它是"在**剩余**大量 import 之前"。

**为什么要这样穿插？**

JavaScript/TypeScript 的 `import` 是同步的——解析器必须加载并执行所有被导入的模块才能继续。这三个函数的导入依赖很轻（各自只需几毫秒），但剩余的 import 链涉及 1,884 个文件，加载大约需要 **~135ms**。

这三行代码在大量模块加载**之前**就触发了两个 I/O 操作：
- `startMdmRawRead()`（`utils/settings/mdm/rawRead.js`）：在 macOS 上生成 `plutil` 子进程读取 MDM 配置文件，在 Windows 上用 `reg query` 读取注册表。这是一次子进程操作，开销不小。
...
main()
  ...
  │   ├── mcp serve → 启动 MCP 服务器模式
  │   └── 无特殊参数 → 继续
  │
  ├── 检查环境变量
  │   ├── CLAUDE_CODE_COORDINATOR_MODE=1 → Coordinator 模式
  │   └── KAIROS → Assistant 模式（ant-only）
  │
...
在 `preAction` hook 中，`init()` 完成之后，系统调用 `runMigrations()`（main.tsx:950）检查 `globalConfig.migrationVersion`。如果不等于当前版本（版本 11），依次运行所有迁移函数：
...
`entrypoints/init.ts` 的 `init()` 函数（line 57）是系统的"内核初始化"，使用 `memoize` 包装确保只执行一次。它按精确顺序启动所有子系统：
...
init()  ← memoized，只执行一次
  ...
  │
  ├── 5. 启动异步后台任务（不等待完成）
  │   ├── 1P 事件日志初始化（动态 import，延迟加载 OTel sdk-logs）
  │   ├── OAuth 账户信息获取
  │   ├── JetBrains IDE 检测
  ...
  │
  ├── 9. initUpstreamProxy()（CCR 模式，feature-gated）
...
- **多个异步任务并行启动**（步骤 5）——不等待完成，在"用户还在看界面"时在后台跑。其中远程设置和策略限制的 loading promise 按条件触发。📚 *课程桥接：并行初始化 = `Promise.all` 的并发模式——多个独立 I/O 操作同时发起，任一完成即可使用其结果，无需串行等待*
...
**设计理念**：这些信息对首屏渲染不是必需的，但对第一次 AI 调用是需要的。把它们放在"用户还在看界面/打字"的时间窗口内预取，到用户真正按 Enter 时，这些信息已经准备好了。

### Git 状态的 5 个并行命令

获取 git 状态不是一个 `git status`——`context.ts` 中的 `getGitStatus()` 通过 `Promise.all` 并行运行 5 个操作：
...
**从敲下命令到看到输入框：~300ms（估算值，缺乏精确 benchmark 数据——源码中有 `profileCheckpoint` 埋点但未公开基准测试结果）。** 对比参考：Go 编译的 gh CLI 启动几乎是瞬间的（<50ms），Python 的 Aider 冷启动约需 1-2s。Claude Code 的 ~300ms 量级对于一个重量级 Node.js CLI 来说是合理的——Pre-import I/O 并行和延迟预取确实减少了感知延迟，但 ~135ms 的模块加载开销仍然是底线，这是 JS/TS 运行时 CLI 工具的固有代价。
...
| 7 个异步后台任务 | `src/entrypoints/init.ts` | :94-132 | 1P 事件日志、OAuth、JetBrains 检测、git 仓库检测、远程设置等并行启动 |
